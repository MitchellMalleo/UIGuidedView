...
- (BOOL)errorExistsInNodeArrayAt:(NSInteger)index {
  if(self.dataSource == nil) {
    NSLog(@"Index does not exist because the dataSource property has not been set");
    return YES;
  } else if (self.nodes == nil) {
    NSLog(@"Index does not exist because nodes have not yet been set");
    return YES;
  } else if (index > self.nodes.count - 1) {
    NSLog(@"Index does not exist because it is greater than the amount of nodes");
    return YES;
  } else if (self.nodes.count == 0) {
    NSLog(@"Index does not exist because the nodes array is empty");
    return YES;
  }
...
- (void)createAnimationToNode:(UIGuidedViewNode *)newNode
                     fromNode:(UIGuidedViewNode *)oldNode
                  inDirection:(UIGuidedViewAnimationDirection)direction
                andSelectNode:(BOOL)shouldSelect {
...
- (UIBezierPath *)shapeLayerPathForIndex:(NSInteger)index {
  return [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.backgroundLinePath.bounds.origin.x + index * self.backgroundLinePath.bounds.size.width / (self.numberOfNodes - 1), self.backgroundLinePath.center.y)
                                        radius:4 startAngle:DEGREES_TO_RADIANS(270)
